...
class ProductSubscriptionOnlinePayment(WebsiteProductSubscription):
    ...
    def get_online_payment_types(self):
        ...
        for acquirer in published_aquirers:
            payment_types.append(
                [acquirer.provider, acquirer.provider.title()]
            )
        return payment_types

    def fill_values(self, values, load_from_user=False):
        values = super(ProductSubscriptionOnlinePayment, self).fill_values(
            values, load_from_user
        )
        values["providers"] = self.get_online_payment_types()
    ...
    def preRenderThanks(self, values, kw):
        ...
        values = super(ProductSubscriptionOnlinePayment, self).preRenderThanks(
            values, kw
        )
