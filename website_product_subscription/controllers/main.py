...
class WebsiteProductSubscription(http.Controller):
    ...
    def fill_values(self, values, load_from_user=False):
        ...
        values["subscriptions"] = sub_temp_obj.sudo().search(
            [("publish", "=", True)]
        )
    ...
    def get_address(self, kwargs):
        address = (
            kwargs.get("street", "") + ", " + kwargs.get("street_number", "")
        )
        vals = {
    ...
    def check_email_confirmation_matches(self, **kwargs):
        ...
        if not is_logged and kwargs.get("email") != kwargs.get(
            "email_confirmation"
        ):
            values = self.fill_values({})
            values.update(kwargs)
        ...
        if not is_logged:
            ...
            if user:
                ...
                return request.website.render(
                    _BECOME_SUBSCRIBER_TEMPLATE, values
                )
    ...
    def generic_form_checks(self, **kwargs):
        ...
        email_missmatch_redirect = self.check_email_confirmation_matches(
            **kwargs
        )
    ...
    @http.route(
        ["/product_subscription/subscribe"],
        type="http",
        auth="public",
        website=True,
    )
            ...
            if kwargs.get("logged") == "on":
                ...
                if not user_obj.user_exist(sponsor.email):
                    user_obj.create_user(
                        {"login": sponsor.email, "partner_id": sponsor.id}
                    )

            sub_req = self.create_subscription_request(
                subscriber_id=subscriber.id, sponsor_id=sponsor.id, **kwargs
            )
