...
class ProductRelease(models.Model):
    ...
    release_qty = fields.Integer(
        string="Product release quantity", required=True, default=1
    )
    ...
    def action_validate(self):
        ...
        self.write({"name": name, "state": "validated"})
    ...
    def action_done(self):
        ...
        for line in self.product_release_lines:
            if line.product_subscription.counter - self.release_qty >= 0:
                ...
                line.product_subscription.counter = (
                    line.product_subscription.counter - self.release_qty
                )  # noqa

        subs_terminated = self.product_release_lines.filtered(
            lambda record: record.product_subscription.counter == 0
        ).mapped("product_subscription")

        subs_terminated.write({"state": "terminated"})

        subs_renew = self.product_release_lines.filtered(
...
class ProductReleaseLine(models.Model):
    ...
    counter = fields.Float(
        related="product_subscription.counter", string="Counter", readonly=True
    )
    picking = fields.Many2one(
        comodel_name="stock.picking", string="Delivery order"
    )
